...
WORKFLOW_PARAMS = {
    "REGION": "us-east-1",
    "VENDOR": "IHS_MARKIT_POLK",
    "FILE_COL_COUNT": 4,
    "FILE_DELIMITER": "|",
    "FILE_NAME": "OUT_POLKNISSAN_v1_20220315.csv",
    "FILE_SCHEMA_NAME": "SCHEMA_HIS_MARKIT_POLK_CARFAX_INBOUND.json",
    ...
    "CLEAN_SNOWPIPE": "SDDBC360.C360_DATA_HS.PIPE_CARFAX_INBOUND_HS",
    "ERROR_SNOWPIPE": "SDDBC360.C360_DATA_HS.PIPE_SNOWPIPE_INGST_ERR",
    "DERIVED_LOAD_PROCEDURE": "C360_DATA_HS.PROC_LOAD_DERIVED_CDC_CARFAX_INBOUND_HS",
...
FILE_DELIMITER = WORKFLOW_PARAMS['FILE_DELIMITER']
FILE_COL_COUNT = WORKFLOW_PARAMS['FILE_COL_COUNT']
...
COMPONENT_NAME = "c360-delimited-job"
...
print('RunId: ', RUN_ID)
print('Vendor: ', VENDOR)
print('File Delimiter: ', FILE_DELIMITER)
print('File Column Count: ', FILE_COL_COUNT)
...
print('File Target URI: ', FILE_TARGET_URI)
...
#################################### Step 1_A - Validation Processing  ####################################
# Loading file from S3 as a text format to check for delimiters
...
# creating User defined function to count delimited records
def count_column(value):
    if value:
        split = value.split(FILE_DELIMITER)
        return len(split)


udf_count_column = udf(count_column, IntegerType())

# ### Apply UDF to count columns available on each row:
df_vld_chk = df.withColumn('COLUMN_COUNT', udf_count_column(df['value']))

#################################### Step 1_B - Validation Processing  ####################################
# This should only get executed when the file validation is successfully completed above else break the job.

# Checking if any record has less or extra columns
df_invalid = df_vld_chk.where(~df_vld_chk.COLUMN_COUNT.contains(FILE_COL_COUNT))

# Terminating job if any invalid row is available
if df_invalid.count() != 0:
    df_invalid.show()
    print("Column count mismatched required column count:" + str(FILE_COL_COUNT))
...
# Schema formation for Delimited file type
...
column_from_schema = schema.select("column").rdd.flatMap(lambda x: x).filter(lambda x: x!= 'Filler').collect()

# Pipe delimited requires escape sequence for separating row correctly
if FILE_DELIMITER == "|":
    FILE_DELIMITER = "\\{}".format(FILE_DELIMITER)

try:
    df_validated = df.select(split(col("value"), FILE_DELIMITER)).rdd.flatMap(lambda x: x).toDF(schema=column_from_schema)
    print("Total Record Count:", df_validated.count())
except Exception as e:
    print("Schema and Dataframe Column Mismatch while dataframe formation")
    raise Exception("Schema and Dataframe Column Mismatch while dataframe formation: ", e)
...
try:
    df_pre_cleansed = df_validated.select(
        *df_columns,
        *[
            (when(df_validated[row["column"]].isNull() | ~df_validated[row["column"]].rlike(str(regexDict[row["dataType"]]) + str("{" + str(row["minLength"]) + "," + str(row["length"]) + "}$")), 1).otherwise(0)).alias('VIO_' + row['column'])
            for row in schemaDict_nullable_f_regex_t
        ],
        *[
            (when(~df_validated[row["column"]].rlike(str(regexDict[row["dataType"]]) + str("{" + str(row["minLength"]) + "," + str(row["length"]) + "}$")), None).otherwise(0)).alias('VIO_' + row['column'])
            for row in schemaDict_nullable_t_regex_t
        ],
        *[
            (lit(0)).alias('VIO_' + str(row['column']))
